...
if (-not ($IsWindows -or $env:OS -match 'Windows')) {
    Write-Host "[INFO] CrystalDiskMark is only available for Windows." -ForegroundColor Yellow
    exit 0
}
...
try {
    ...
    Write-Host "  [i] Please install $AppName manually from https://crystalmark.info/en/software/crystaldiskmark/" -ForegroundColor Cyan
