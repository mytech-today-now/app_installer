...
if (-not ($IsWindows -or $env:OS -match 'Windows')) {
    Write-Host "[INFO] VSDC Video Editor is only available for Windows." -ForegroundColor Yellow
    exit 0
}

$AppName = "VSDC Free Video Editor"
$WingetId = "FlashIntegro.VSDCFreeVideoEditor"
...
try {
    ...
    Write-Host "  [i] Please install $AppName manually from https://www.videosoftdev.com/" -ForegroundColor Cyan
