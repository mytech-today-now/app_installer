...
try {
    ...
    $result = winget install --id Piriform.Speccy --silent --accept-source-agreements --accept-package-agreements 2>&1

    if ($LASTEXITCODE -eq 0) {
        Write-Host "  [OK] Speccy installed successfully!" -ForegroundColor Green
        exit 0
    }
    else {
    ...
    Write-Host "Error installing Speccy: $_" -ForegroundColor Red
    exit 1
}
