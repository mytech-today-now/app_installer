# Npcap Installation Script
# Part of myTech.Today Application Installer Suite
# Windows-only: Npcap is not available on macOS or Linux.
...
if (-not ($IsWindows -or $env:OS -match 'Windows')) {
    Write-Host "[INFO] Npcap is only available for Windows." -ForegroundColor Yellow
    exit 0
}
...
try {
    ...
    Write-Host "  [i] Please install $AppName manually from https://npcap.com/" -ForegroundColor Cyan
