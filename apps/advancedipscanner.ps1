...
try {
    Write-Host "Installing Advanced IP Scanner..." -ForegroundColor Cyan
    ...
    $result = winget install --id Famatech.AdvancedIPScanner --silent --accept-source-agreements --accept-package-agreements 2>&1

    if ($LASTEXITCODE -eq 0) {
        Write-Host "  [OK] Advanced IP Scanner installed successfully!" -ForegroundColor Green
        exit 0
    }
    else {
    ...
    Write-Host "Error installing Advanced IP Scanner: $_" -ForegroundColor Red
    exit 1
}
