...
try {
    Write-Host "Installing TreeSize Free..." -ForegroundColor Cyan
    ...
    $result = winget install --id JAMSoftware.TreeSize.Free --silent --accept-source-agreements --accept-package-agreements 2>&1

    if ($LASTEXITCODE -eq 0) {
        Write-Host "  [OK] TreeSize Free installed successfully!" -ForegroundColor Green
        exit 0
    }
    else {
    ...
    Write-Host "Error installing TreeSize Free: $_" -ForegroundColor Red
    exit 1
}
