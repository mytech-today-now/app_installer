...
if (-not ($IsWindows -or $env:OS -match 'Windows')) {
    Write-Host "[INFO] CPU-Z is only available for Windows." -ForegroundColor Yellow
    exit 0
}
...
try {
    ...
    Write-Host "  [i] Please install $AppName manually from https://www.cpuid.com/softwares/cpu-z.html" -ForegroundColor Cyan
