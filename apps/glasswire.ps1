...
try {
    ...
    $result = winget install --id GlassWire.GlassWire --silent --accept-source-agreements --accept-package-agreements 2>&1

    if ($LASTEXITCODE -eq 0) {
        Write-Host "  [OK] GlassWire installed successfully!" -ForegroundColor Green
        exit 0
    }
    else {
    ...
    Write-Host "Error installing GlassWire: $_" -ForegroundColor Red
    exit 1
}
