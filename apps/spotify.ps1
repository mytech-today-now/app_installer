...
.DESCRIPTION
    This script installs Spotify using winget package manager.
...
try {
    ...
    $result = winget install --id Spotify.Spotify --silent --accept-source-agreements --accept-package-agreements 2>&1

    if ($LASTEXITCODE -eq 0) {
        Write-Host "  [OK] Spotify installed successfully!" -ForegroundColor Green
        exit 0
    }
    else {
