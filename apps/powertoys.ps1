...
try {
    ...
    $result = winget install --id Microsoft.PowerToys --silent --accept-source-agreements --accept-package-agreements 2>&1

    if ($LASTEXITCODE -eq 0) {
        Write-Host "  [OK] PowerToys installed successfully!" -ForegroundColor Green
        exit 0
    }
    else {
    ...
    Write-Host "Error installing PowerToys: $_" -ForegroundColor Red
    exit 1
}
