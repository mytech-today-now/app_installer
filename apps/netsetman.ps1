...
if (-not ($IsWindows -or $env:OS -match 'Windows')) {
    Write-Host "[INFO] NetSetMan is only available for Windows." -ForegroundColor Yellow
    exit 0
}

$AppName = "NetSetMan"
$WingetId = "NetSetMan.NetSetMan"
...
try {
    ...
    Write-Host "  [i] Please install $AppName manually from https://www.netsetman.com/" -ForegroundColor Cyan
