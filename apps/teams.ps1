...
.DESCRIPTION
    This script installs Microsoft Teams using winget package manager.
...
try {
    ...
    $result = winget install --id Microsoft.Teams --silent --accept-source-agreements --accept-package-agreements 2>&1

    if ($LASTEXITCODE -eq 0) {
        Write-Host "  [OK] Microsoft Teams installed successfully!" -ForegroundColor Green
        exit 0
    }
    else {
