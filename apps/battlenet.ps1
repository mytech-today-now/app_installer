<#
.SYNOPSIS
    Installs Battle.net game launcher.

.DESCRIPTION
    This script installs Battle.net using winget package manager.

.NOTES
    File Name      : battlenet.ps1
    Author         : myTech.Today
    Version        : 1.0.0
...
try {
    ...
    $result = winget install --id Blizzard.BattleNet --silent --accept-source-agreements --accept-package-agreements 2>&1

    if ($LASTEXITCODE -eq 0) {
        Write-Host "  [OK] Battle.net installed successfully!" -ForegroundColor Green
        exit 0
    }
    else {
