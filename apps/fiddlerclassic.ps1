...
if (-not ($IsWindows -or $env:OS -match 'Windows')) {
    Write-Host "[INFO] Fiddler Classic is only available for Windows." -ForegroundColor Yellow
    exit 0
}
...
try {
    ...
    Write-Host "  [i] Please install $AppName manually from https://www.telerik.com/fiddler/fiddler-classic" -ForegroundColor Cyan
