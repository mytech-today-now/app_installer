...
.DESCRIPTION
    ...
    Windows-only: Visual C++ Redistributable is not available on macOS or Linux.

.NOTES
    File Name      : vcredist.ps1
    Author         : myTech.Today
    Version        : 1.0.0
...
if (-not ($IsWindows -or $env:OS -match 'Windows')) {
    Write-Host "[INFO] Visual C++ Redistributable is only available for Windows." -ForegroundColor Yellow
    exit 0
}
...
try {
    ...
    $result = winget install --id Microsoft.VCRedist.2015+.x64 --silent --accept-source-agreements --accept-package-agreements 2>&1
    ...
    Write-Host "Error installing Visual C++ Redistributable: $_" -ForegroundColor Red
    exit 1
}
